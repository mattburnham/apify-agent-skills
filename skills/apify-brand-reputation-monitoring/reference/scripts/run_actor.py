...
Usage:
    ...
    # Export to file
    uv run scripts/run_actor.py --actor ACTOR_ID --input '{}' --output reviews.csv --format csv

    # Export basic fields only
    uv run scripts/run_actor.py --actor ACTOR_ID --input '{}' --output reviews.csv --format csv --fields basic
"""
...
Examples:
  ...
  uv run scripts/run_actor.py \\
    ...
    --input '{"placeUrls": ["https://www.google.com/maps/place/..."], "maxReviews": 50}'

  # Export all data to CSV
  uv run scripts/run_actor.py \\
    --actor "maxcopell/tripadvisor-reviews" \\
    --input '{"startUrls": [{"url": "https://www.tripadvisor.com/..."}]}' \\
    --output reviews.csv --format csv

  # Export basic fields only
  uv run scripts/run_actor.py \\
    --actor "apify/facebook-reviews-scraper" \\
    --input '{"startUrls": ["https://www.facebook.com/..."]}' \\
    --output reviews.csv --format csv --fields basic
        """,
    )
    parser.add_argument(
        "--actor",
        required=True,
        help="Actor ID (e.g., compass/Google-Maps-Reviews-Scraper)",
    )
    parser.add_argument(
        "--input",
        required=True,
