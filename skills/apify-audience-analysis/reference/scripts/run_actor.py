...
USER_AGENT = "apify-agent-skills/apify-audience-analysis-1.0.0"
...
Examples:
  ...
  uv run scripts/run_actor.py \\
    ...
    --input '{"usernames": ["apify"]}'

  # Export all data to CSV
  uv run scripts/run_actor.py \\
    --actor "clockworks/tiktok-followers-scraper" \\
    --input '{"profiles": ["username"]}' \\
    --output followers.csv --format csv

  # Export basic fields only
  uv run scripts/run_actor.py \\
    --actor "streamers/youtube-channel-scraper" \\
    --input '{"startUrls": [{"url": "https://youtube.com/@channel"}]}' \\
    --output audience.csv --format csv --fields basic
        """,
    )
    parser.add_argument(
        "--actor",
