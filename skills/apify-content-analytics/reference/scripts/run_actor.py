...
USER_AGENT = "apify-agent-skills/apify-content-analytics-1.0.0"
...
Examples:
  ...
  uv run scripts/run_actor.py \\
    ...
    --input '{"directUrls": ["https://www.instagram.com/p/ABC123/"]}'

  # Export all data to CSV
  uv run scripts/run_actor.py \\
    --actor "apify/instagram-reel-scraper" \\
    --input '{"directUrls": ["https://www.instagram.com/reels/ABC123/"]}' \\
    --output analytics.csv --format csv

  # Export basic fields only
  uv run scripts/run_actor.py \\
    --actor "clockworks/tiktok-scraper" \\
    --input '{"profiles": ["username"]}' \\
    --output analytics.csv --format csv --fields basic
        """,
    )
    parser.add_argument(
        "--actor",
        required=True,
        help="Actor ID (e.g., apify/instagram-post-scraper)",
    )
    parser.add_argument(
        "--input",
        required=True,
