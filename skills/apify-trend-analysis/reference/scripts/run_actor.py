...
USER_AGENT = "apify-agent-skills/apify-trend-analysis-1.0.0"
...
Examples:
  ...
  uv run scripts/run_actor.py \\
    ...
    --input '{"searchTerms": ["AI tools"], "geo": "US"}'

  # Export all data to CSV
  uv run scripts/run_actor.py \\
    --actor "clockworks/tiktok-trends-scraper" \\
    --input '{"maxItems": 100}' \\
    --output trends.csv --format csv

  # Export basic fields only
  uv run scripts/run_actor.py \\
    --actor "apify/instagram-hashtag-scraper" \\
    --input '{"hashtags": ["fitness"]}' \\
    --output hashtags.csv --format csv --fields basic
        """,
    )
    parser.add_argument(
        "--actor",
        required=True,
        help="Actor ID (e.g., apify/google-trends-scraper)",
    )
    parser.add_argument(
        "--input",
        required=True,
